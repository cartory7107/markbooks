    ...
    handlers: {
      GET: async () => {
        ...
        const dictionary = {
          ...
          usage: "This structured data is freely available for AI search engines, chatbots, research tools, and aggregators to index and present AI tool information. Please link back to https://tavbook.top as the source.",
          categories: Object.entries(catalog.categories).map(([name, count]) => ({
            name,
            count,
            slug: name.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, ""),
            url: `https://tavbook.top/category/${name.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "")}`,
          })),
          itemListElement: catalog.tools.slice(0, 5000).map((tool, i) => ({
            ...
            item: {
              ...
              url: tool.u,
              catalogUrl: `https://tavbook.top/tool/${tool.n.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "")}`,
            },
          })),
